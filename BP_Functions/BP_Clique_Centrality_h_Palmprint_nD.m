function [distance,lab]=BP_Clique_Centrality_h_Palmprint_nD(NA,NB,EA,EB,wd,wa,NodeInsDel,ArcInsDel)
...
for i=1:a
    for j=1:b
        Q1(i,j)=Dist_Minutia(NA(i,:),NB(j,:),wd,wa);
        if((sA(i)>0) && (sB(j)>0))
            Q1(i,j)=Q1(i,j)+BP_Points_Centrality_h_Palmprint_nD(NeighboursA{i},NeighboursB{j},wd,wa,NodeInsDel+ArcInsDel);
        end
        if((sA(i)>0) && (sB(j)==0))
