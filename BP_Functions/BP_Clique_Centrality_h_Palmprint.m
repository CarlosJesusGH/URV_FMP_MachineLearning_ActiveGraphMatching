function [distance,lab]=BP_Clique_Centrality_h_Palmprint(NA,NB,EA,EB,NodeInsDel,ArcInsDel)
...
for i=1:a
    for j=1:b
        ...
        if((sA(i)>0) && (sB(j)>0))
            Q1(i,j)=Q1(i,j)+BP_Points_Centrality_h_Palmprint(NeighboursA{i},NeighboursB{j},NodeInsDel+ArcInsDel);
        end
        if((sA(i)>0) && (sB(j)==0))
