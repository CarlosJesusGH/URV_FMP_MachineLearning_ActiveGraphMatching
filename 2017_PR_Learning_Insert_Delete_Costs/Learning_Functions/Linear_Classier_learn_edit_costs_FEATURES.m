function [Kv, Ke, data1, data2] = Linear_Classier_learn_edit_costs_FEATURES(data, Outlier,Imposed_Kv,Imposed_Ke)
    ...
    for elem = 1:size(data,2)  
        ...
        for label = 1:length(labelling)
            ...
            if(labelling(label)>0)
                ...
                [costStar , nn]  = computeCostStar_FEATURES(G1, G2, label, labelling(label),Features_Distance);
            ...
            % calculating deletion points (second option)
                ...
                for k = 1:length(G2.nodes(:,1))
                    ...
                    [costStar , nn]  = computeCostStar_FEATURES(G1, G2, label, k,Features_Distance);
