function [Kv, Ke, data1, data2] = Linear_Classier_learn_edit_costs_PALMPRINT(data, Outlier,Imposed_Kv,Imposed_Ke)
...
%         elem
        ...
        for label = 1:length(labelling)
            ...
            if(labelling(label)>0)
                ...
                costStar  = computeCostStar_PALMPRINT(G1, G2, label, labelling(label));
                DDD=n-abs(n-m)+1;
                if (DDD ~= 0)
                    yy1(end+1) = (Dist_Minutia(G1.nodes(label,1:3),G2.nodes(labelling(label),1:3),1,1)+costStar) / DDD; 
                    xx1(end+1) = (DDD-1)  / DDD; 
                end
            else
            ...
            % calculating deletion points (second option)
                ...
                for k = 1:length(G2.nodes(:,1))
                    ...
                    costStar  = computeCostStar_PALMPRINT(G1, G2, label, k);
                    DDD=n-abs(n-m)+1;
                    
                    if (DDD ~= 0)
                         yy2(end+1) = (Dist_Minutia(G1.nodes(label,1:3),G2.nodes(k,1:3),1,1)+ costStar) / DDD; 
                         xx2(end+1) = (DDD-1) / DDD; 
                    end
               end
               if(~isempty(yy2))
