    ...
    for elem = 1:size(data,2)    
        ...
        for label = 1:length(labelling)
             ...
             if(labelling(label)>0)
                ...
                costStar = computeCostStar_nD(G1, G2, label, labelling(label));
                nn=abs(n-m);
                zv=n-nn+1;
                ze=zv-1;
                if (zv ~= 0)
                    z1=(abs(G1.nodes(label,:)-G2.nodes(labelling(label),:)))+costStar';
                    s1(1)=z1(1)/zv;
                    s1(2:end)=(z1(2:end)-z1(1))/zv;  %S1
                    s1(end+1)=ze/zv; %Se
            ...
            % calculating deletion points (second option)
                ...
                for k = 1:length(G2.nodes(:,1))
                    s2=zeros(1,T);   
                    m = sum(G2.edges(k,:));
                    costStar = computeCostStar_nD(G1, G2, label, k);
                    nn=abs(n-m);              
                    zv=n-nn+1;
                    ze=zv-1;
                    if (zv ~= 0)
                        z2=(abs(G1.nodes(label,:)-G2.nodes(k,:)))+costStar';
                        s2(1)=z2(1)/zv;
                        s2(2:end)=(z2(2:end)-z2(1))/zv; %S2
                        s2(end+1)=ze/zv; %Se
