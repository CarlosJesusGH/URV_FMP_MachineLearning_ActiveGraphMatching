% ***** Learn weights with the nimimisation method ****
% *****  reference [37] *****
close all;
clc;
clear;
...
display('ROTATIONZOOM: Minimisation method: Learn weights');
Database='ROTATIONZOOM'; Kv_ini=1; Ke_ini=1; lambda=0.0001; Selected_Registers= [4 20 36 52 68] ;
varname=[Database '.mat'];
load(varname);
DB_Learning=LoadAllRegisters_Learning_Bijective_Selected_Registers(Database,Selected_Registers);
a=DB_Learning{1}.graph1.nodes;
num_features=size(a,2)-2; % x,y position is not considered.
w=(1/num_features)*ones(num_features,1);

w = Minimisation_learn_weights_FEATURES(DB_Learning,w,lambda);
save('../2016_TARRAGONA_GRAPH_DATABASE/W_ROTATIONZOOM_MINIMISATION','w');
save('W_ROTATIONZOOM_MINIMISATION','w');
