function [distance,lab]=BP_Clique_Centrality_h_With_Weights_Features(NA,NB,EA,EB,w,attributes,NodeInsDel,ArcInsDel)
...
Features_Distance=Compute_Features_Distance_weights(NA,NB,w');  
Q1=zeros(a,b);
for i=1:a
    for j=1:b
        Q1(i,j)=Features_Distance(i,j);
        if((sA(i)>0) && (sB(j)>0))
            Q1(i,j)=Q1(i,j)+BP_Points_Centrality_h_Features(i,j,Features_Distance,EA,EB,NodeInsDel+ArcInsDel);
        end
        if((sA(i)>0) && (sB(j)==0))
