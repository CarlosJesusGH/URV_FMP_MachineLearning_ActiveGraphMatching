function [distance,lab]=SFBP_Eigenvector_Centrality_jv(NA,NB,EA,EB,NodeInsDel,Beta)
% francesc.serratosa@urv.cat
% [Distance,labelling]=SFBP_Planar_Centrality_jv(NA,NB,EA,EB,NodeInsDel,ArcInsDel)
...
for i=1:a
    for j=1:b
        ...
        if(sA(i) == 0 && sB(j) == 0)
            ...
            if((sA(i)>0) && (sB(j)==0))
                Q1(i,j)=Beta*Q1(i,j)+ (1-Beta)*abs(CentralityA(i)-CentralityB_Null); %CentralityA(i);
            end
            if((sA(i)==0) && (sB(j)>0))
                Q1(i,j)=Beta*Q1(i,j)+ (1-Beta)*abs(CentralityA_Null-CentralityB(j)); %CentralityB(j);
            end
        end     
    end
end
