function [distance,lab]=BP_Clique_Centrality_h_matched(NA,NB,EA,EB,NodeInsDel,ArcInsDel, matched1, matched2)
...
% CJG_20170328: force values imposed by de user
% ---
for i=1:length(matched1)    
    % If it is a sustitution
    if matched1(i) ~= 0 && matched2(i) ~= 0
        C(matched1(i),:) = Nul;
        C(:,matched2(i)) = Nul;
        C(matched1(i),matched2(i)) = 0;
    end
    % If it is an insertion
    if matched1(i) == 0 && matched2(i) ~= 0
        C(matched2(i),:) = Nul;
        C(:,b + matched2(i)) = Nul;
        C(matched2(i),b + matched2(i)) = 0;
    end
    % If it is a deletion
    if matched1(i) ~= 0 && matched2(i) == 0
        C(a + matched1(i),:) = Nul;
        C(:,matched1(i)) = Nul;
        C(a + matched1(i),matched1(i)) = 0;
    end
    % If both are nulls
    if matched1(i) == 0 && matched2(i) == 0
        % Do nothing            
