    ...
    for elem = 1:size(data,2)    
        ...
        for label = 1:length(labelling)
            ...
            if(labelling(label)>0)
                ...
                costStar  = computeCostStar_FEATURES(G1, G2, label, labelling(label), labelling,Features_Distance);
            ...
            % calculating deletion points (second option)
               for k = 1:length(G2.nodes(:,1))
                    ...
                    costStar  = computeCostStar_FEATURES(G1, G2, label, k, labelling,Features_Distance);
