function [distance,lab]=FBP_Clique_Centrality_h(NA,NB,EA,EB,NodeInsDel,ArcInsDel)
% francesc.serratosa@urv.cat
% [Distance,labelling]= FBP_Clique_Centrality_h(NA,NB,EA,EB,NodeInsDel,ArcInsDel)
...
for i=1:a
    for j=1:b
        ...
        if((sA(i)>0) && (sB(j)>0))
            Q1(i,j)=Q1(i,j)+FBP_Points_Centrality_h(NeighboursA{i},NeighboursB{j},NodeInsDel+ArcInsDel)-(ArcInsDel+NodeInsDel)*(sA(i)+sB(j));
        end
    end
end
lab=Hungarian(Q1);
...
C_ve_di=NodeInsDel*(a+b)+(ArcInsDel+NodeInsDel)*(sum(sA)+sum(sB));
